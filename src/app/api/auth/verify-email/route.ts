...
export async function GET(request: NextRequest): Promise<NextResponse<ApiResponse> | Response> {
  try {
    ...
    // Token is stored as "OTP:MAGIC_TOKEN" - we need to match the magic token part
...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse<UserResponse>>> {
  try {
    ...
    const response = NextResponse.json({
      ...
      token: jwtToken, // Include token in response for frontend localStorage
    });

    setAuthCookieOnResponse(response, jwtToken);
    return response;
  } catch (error: any) {
    console.error('Email verification error:', error);
    return NextResponse.json(
