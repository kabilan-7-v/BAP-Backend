...
import { formatUserResponse, generateOTPFromToken } from '@/utils/helpers';
...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse<UserResponse>>> {
  try {
    ...
    // Generate expected OTP from token
    const expectedOtp = generateOTPFromToken(user.emailVerificationToken);
